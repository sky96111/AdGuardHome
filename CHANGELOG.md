...
- The statistics are now stored in an SQLite database instead of bbolt, using the same pure-Go driver.  This removes the full-decode-and-aggregate pass over every hour unit on each dashboard request, which previously also blocked the DNS request processing, and removes the locking on the DNS request path.  The previous `stats.db` format isn't compatible; delete the old file manually to reset the statistics, otherwise AdGuard Home reports it and refuses to start.  The statistics are now flushed every minute, and a snapshot of the current hour is refreshed every five minutes, so a crash loses at most five minutes of the collected time counters and at most an hour of the top lists, where it previously could lose up to an hour of everything.  The top lists are also recomputed from the per-hour data on startup, so restarts and crashes can no longer skew them.
